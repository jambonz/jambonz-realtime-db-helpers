 ...
 */
async function synthAudio(client, logger, stats, {vendor, language, voice, text}) {
  ...
  if (!result) {
    ...
    audioBuffer = 'google' === vendor ?
      await synthGoogle(stats, language, voice, text) :
...
const synthGoogle = async(stats, language, voice, text) => {
